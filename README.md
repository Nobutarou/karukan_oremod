...
https://github.com/togatoga/karukan を自分の好みに合わせて変更しようとしているものです。
SKK に慣れすぎているので、そこを慣れるまで吸収するためのものです。

ちなみに Karukan は、ライブ変換オフにしておいても、どんどん変換候補が出てくるので、ガシガ
シ確定していける SKK のキーバインドと相性が良さそうな気がします。

Github のお作法とか分からないので、フォーク機能とか使ってません。参考にしてもらえたら嬉し
いですが、Issue とかプルリクとか送られても対処する能力がないです。

ライセンスは本家のライセンスと同じものを引き継ぎます。

## 変更点

- ctrl-j で確定
  - SKK に慣れてるので「わたしは」ではなく「わたし」「は」というような打ち方をしてしまう。
  - 「は」でスペースで変換モードに入ったあと、次の文字を入力すれば「は」で確定はするが、そ
    もそも、そのままで良いものに対し、変換モードに入りたくない
  - エンターは、送信や改行が入ってしまうのが怖くて、すごくストレス。
  - ライブ変換が賢くなれば、ctrl-j だけで、ガンガン入力が進む。
- shift で英数モードに入るのを止める。
  - fcitx5 にとっては IM の入力は全て未確定な入力のため、LibreCAD などでのショートカットが
    動かなくなる。
  - 英数字は IM オフで入力した方が、確実。
- スペースは半角にする。
- 記号、数字は、変換モードに入る。
  - 1番目の入力の場合は確定。
    - 。、「」以外はそのまま全角での入力。
    - 頻繁に必要なものは [ここ](./karukan-im/src/core/engine/input.rs) の ``pub(super) fn
      process_key_empty(`` を修正すればいい。
    - emoji.yml を変更して、そっちでやるのも一手。

## 変更したかったけど
- q でひらがな、カタカナ行ったり来たりしたい。
  - くえsちおn → クエスチョンとかやりたいから、やめた。
- l で確定したり、アルファベットードに入ったりとか考えたけど、ぃふぇ → ライフとかやれなく
  なるからやめた
- 大文字入力で変換モードに入れるように頑張ってみたけど、実力が足りなかった。
- /stress -> ストレスとかやりたかった
  - :smile の顔文字のやつを使えば同じことがやれる。毎回ビルドが必要だけど。
  - [emoji.yml](./karukan-engine/data/emoji.yml)
